...
export function CheckINModal ({closeFunc, id = null, parentID = null} : CheckINProps) {
    ...
    useEffect(() => {
        if (currentData) {
            ...
            editorHandler(currentData.title);
    ...
    const editorHandler = (value: string) => setEditorValue(value);
    ...
    return (
        ...
        >
            {/* заголовок */}
            <div className={`d-flex flex-row justify-content-between align-items-start border-bottom ${cl.header}`}>
                <div className={cl.title}>
                    <div>Check-in по задаче {currentTask?.id ? `"${currentTask.title}"`: null}</div>
                    <StarList classList={cl.titleStars} status={getStarStatus(historyList[0]?.ciStatus)} value={currentTask?.complexity ?? 0} />
                </div>
            ...
            <Modal.Body className={`${cl.contentContainer}`}>
                <div className={`${cl.gridContainer}`}>
                    <div className={`${cl.leftContainer}`}>
                        ...
                        </Field>
                        {
                            ...
                            <div className={cl.progressInputs}>
                                ...
                                <div className={cl.progressInputContainer}>
                                    ...
                                    handlerInput={(value) => {changeCurrentProcent(value)}}
                                    iconRigth={<>%</>} />
                                </div>
                            </div>
                        }
                            ...
                            label={
                                <>
                                    ...
                                    -&nbsp;&nbsp;
                                    {
                                        ...
                                        ?
                                        getProgressPlanColor(lastProcent, new Date(moment(currentTask.dueDate, 'DD.MM.YYYY').format('YYYY-MM-DD')), new Date(moment(currentTask.createDate, 'DD.MM.YYYY').format('YYYY-MM-DD'))).planValue + '%'
                                        : 
                        ...
                        >
                            <div>
                                <ReactQuill theme="snow" value={editorValue} onChange={editorHandler} className={`${cl.editor}`}/>
                            </div>
                        </Field>
                    </div>
                    <div className={cl.rightContainer}>
                        <div className={cl.historyTitle}>История:</div>
                        <div className='customScroll' style={{maxHeight: '80%', height: '100%', minHeight: 'fit-content'}}>
                            <div className={cl.historyList}>
                                {historyList.map(item => {
                                    let statusColor = bgThemeEnum.success;
                                    if (item.ciStatus == CheckinStatus.danger) statusColor = bgThemeEnum.danger;
                                    if (item.ciStatus == CheckinStatus.warning) statusColor = bgThemeEnum.warning;

                                    const [start, end] = item.cd ? getWeekStartEndDate(
                                        Number(item.cd.split('-')[1]), 
                                        item.cd.split('-')[0], 
                                        'DD.MM.YYYY'
                                    ) : ["_", "_"];

                                    let weekDay = item.cd && item.cd.split('-').reverse().map((el, index) => {
                                        if (index == 0) {
                                            if (el.startsWith('0')) {
                                                el = el.substring(1)
                                            }
                                        ...
                                        <div key={item.id ?? item.cd} className={cl.historyItem}>
                                            <div className={cl.historyItemHeader}>
                                                <div className={cl.historyItemTitle}>
                                                    ...
                                                    {weekDay}&nbsp;
                                                    <span className={cl.historyItemTitleStartEnd}>({start} - {end})</span>
                                                </div>
                                                ...
                                                {item.id && (
                                                    <div className={cl.historyItemStats}>
                                                        ...
                                                        {Number(item.progresschange) !== Number(item.procent) && (
                                                            <div className={cl.historyItemProgress}>
                                                                {item.progresschange ?? 0}% -{'>'} {item.procent ?? 0}%
                                                            </div>
                                                        )}
                                                    </div>
                                                )}
                                            ...
                                            {item.id && (
                                                <div className={cl.historyItemBody}>
                                                    {item.title.replace(/<[^>]+>/g, '').length
                                                        ...
                                                        : <div className={`${cl.historyItemComment} ${cl.historyItemCommentNoComment}`}>Без комментария</div>
                                                    }
                                                    
                                                    <div className={cl.historyItemUserInfo}>
                                                        <AvatarItem className={cl.historyItemUserAvatar} login={item.updateLogin} fio={item.updateFIO} />
                                                        <div className={cl.historyItemDateComment}>{item.updateDate}</div>
                                                    </div>
                                                </div>
                                            )}

                                            {!item.id && (
                                                <div className={`${cl.historyItemComment} ${cl.historyItemCommentNoCheckin}`}>
                        ...
                        {historiesLength > 0 && (
                            <div className='d-flex flex-row justify-content-center'>
                                ...
                                >
                                    <i className="ri-arrow-down-line me-2" style={{verticalAlign: 'middle'}}></i>
                                    Загрузить еще 
                                    {historyReloading && (
            ...
            <div className={`d-flex flex-row justify-content-between align-items-center pt-3 pb-3 ${cl.modalFooter}`}>        
                <div></div>
                <div>
                    <Button
                        onlyIcon={true}
